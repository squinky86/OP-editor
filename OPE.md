...
**Implementation note — click the lane, and also click the words.** The lanes shipped read-only, so `Ctrl+B` on a selected note was the only way to place a break; the ruler showed the result and did nothing. Clicking a lane now toggles a break in it, snapped to the nearest note boundary on the measure's spacing grid (§7.1), with a dotted ghost under the pointer showing where it will land before the click commits to it. Dragging an existing marker is still not implemented — click-to-remove plus click-to-add covers moving one, at two clicks instead of a drag.

The more useful half turned out to be somewhere else. A ruler answers "at which tick?", but the question an author actually has is "after which *word*?" — so the alignment grid grew a **`break` row** above the note row, one cell per lyric slot, marked at the right-hand edge of the syllable the break follows: `│` blue required, `┆` amber optional, `╎` grey non-breaking, coloured to match the ruler. Click to toggle a required break, right-click for the lane menu. Because the grid is per voice while breaks are song-wide, a break that lands between the displayed voice's *notes* rather than between two of its *syllables* is drawn hatched instead of solid — R9.4 and R9.5 shown where the words are rather than as a line in the Problems panel.

Both surfaces call the same three operations on the session (`phraseBreakAt`, `setPhraseBreak`, `togglePhraseBreak`) rather than reaching into the three fields themselves, which is what keeps a break in exactly one lane and keeps a translation from freezing the lanes it did not touch: an overlay's `phrase_breaks` replaces the base's array whole, so an edit starts from the merged list and writes only the lane that changed.
